...
import { getStagingRequests } from "../../actions/superAdmin";

export default function StagingRequestsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = await getStagingRequests();
        setStagingRequests(data);
        setLoading(false);
      } catch (error) {
  ...
  const getColumnNames = () => {
    ...
    const priorityColumns = ['requestId', 'date', 'selectedDepartment', 'workDescription', 'status'];
  ...
  return (
    <div className="container mx-auto px-4">
      <h1 className="text-2xl font-bold mb-4 text-black" style={{ color: '#1e3a8a' }}>Staging Requests</h1>
      ...
      <div className="mb-4 flex flex-col md:flex-row gap-4">
        <input
          ...
          style={{ borderColor: '#1e3a8a', boxShadow: 'focus ? 0 0 0 3px rgba(30, 58F, 138, 0.3) : none' }}
          value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-black">No staging requests found.</div>
      ) : (
            ...
            <tbody>
              {filteredRequests.map((request) => (
                <tr key={request.requestId} 
                  ...
                  {getColumnNames().map(column => (
                    <td key={column} className="py-2 px-4 border-b overflow-hidden text-ellipsis text-black">
                      {column === 'availed' ? (
                        <span 
