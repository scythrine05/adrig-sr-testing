...
import { getSanctionedRequests } from "../../actions/superAdmin";

export default function SanctionedRequestsPage() {
  const [sanctionedRequests, setSanctionedRequests] = useState([]);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = await getSanctionedRequests();
        setSanctionedRequests(data);
        setLoading(false);
      } catch (error) {
  ...
  const filteredRequests = sanctionedRequests.filter((request) =>
    request.requestId.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (request.workDescription && request.workDescription.toLowerCase().includes(searchTerm.toLowerCase())) ||
    (request.RequestCreator?.name && request.RequestCreator.name.toLowerCase().includes(searchTerm.toLowerCase())) ||
    (request.RequestCreator?.username && request.RequestCreator.username.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  const getColumnNames = () => {
    ...
    const priorityColumns = ['requestId', 'date', 'sanctionedDate', 'selectedDepartment', 'workDescription', 'workType'];
  ...
  const formatHeaderText = (text) => {
    ...
    if (text === 'sanctionedDate') return 'Sanctioned Date';
    if (text === 'sanctionedBy') return 'Sanctioned By ID';
  ...
  return (
    <div className="container mx-auto px-4">
      <h1 className="text-2xl font-bold mb-4 text-black" style={{ color: '#1e3a8a' }}>Sanctioned Requests</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-black">No sanctioned requests found.</div>
      ) : (
            ...
            <tbody>
              {filteredRequests.map((request) => (
                ...
                >
                  ...
                  <td className="py-2 px-4 border-b text-black">{request.RequestCreator?.name || 'N/A'}</td>
                  <td className="py-2 px-4 border-b text-black">{request.SanctionedBy?.name || 'N/A'}</td>
                </tr>
              ))}
            </tbody>
          </table>
