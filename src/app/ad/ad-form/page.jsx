...
const SearchForm = () => {
  ...
  const separateRequestsByCorridorType = (requests) => {
    ...
    const nonCorridor = requests.filter(req => req.corridorType === "non-corridor");
    const emergency = requests.filter(req => req.corridorType === "emergency");
  ...
  const filteredAndSortedRequests = React.useMemo(() => {
    ...
    const nonCorridor = result.filter(req => req.corridorType === "non-corridor");
      ...
      <div className="overflow-x-auto relative">
        ...
        <div className={`hidden md:block ${isFullScreen ? 'fixed inset-0 z-50 bg-white p-4' : ''}`}>
          ...
          <div className={`${isFullScreen ? 'h-[calc(100vh-120px)] overflow-auto' : ''}`}>
            ...
            <h2 className="text-2xl font-bold text-blue-700 my-5">Corridor Requests</h2>
                    ...
                    >
                      <div className="flex items-center gap-1 justify-between">
                        ...
                        <IconButton 
                          ...
                          className="p-0.5"
                        >
                          <FilterListIcon fontSize="small" color={filters[column.id] ? "primary" : "inherit"} />
                        </IconButton>
                      </div>
                    </th>
                  ))}
                ...
                {corridorRequests.length > 0 ? (
                  corridorRequests.map((request) => (
                    <tr key={request.requestId} className="hover:bg-gray-50">
                      ...
                      <td className="border border-gray-300 p-3 whitespace-nowrap">{request.corridorType}</td>
            ...
            <h2 className="text-2xl font-bold text-green-700 my-5">Non-Corridor Requests</h2>
            <table className="w-full border-collapse border border-gray-300 mb-10">
              <thead>
                <tr className="">
                  {[
            ...
            {emergencyRequests.length > 0 && (
              <>
                <h2 className="text-2xl font-bold text-red-600 my-5">Emergency Block Requests</h2>
        ...
        >
          <div className="p-3 max-h-[300px] overflow-y-auto" style={{ minWidth: '200px' }}>
            ...
            <div className="mb-2">
              <div className="flex gap-2 mb-1">
                <button 
                  className={`text-sm py-1 px-2 rounded flex items-center gap-1 ${
                    sortConfig.key === currentFilterColumn && sortConfig.direction === 'ascending' 
                      ? 'bg-blue-100 text-blue-700' 
                      : 'hover:bg-gray-100'
                  }`}
                  onClick={() => handleSortFromFilter('ascending')}
                >
                  <ArrowUpwardIcon fontSize="small" /> Sort A-Z
                </button>
                <button 
                  className={`text-sm py-1 px-2 rounded flex items-center gap-1 ${
                    sortConfig.key === currentFilterColumn && sortConfig.direction === 'descending' 
                      ? 'bg-blue-100 text-blue-700' 
                      : 'hover:bg-gray-100'
                  }`}
                  onClick={() => handleSortFromFilter('descending')}
                >
                  <ArrowDownwardIcon fontSize="small" /> Sort Z-A
                </button>
              </div>
            </div>
            ...
            <div className="mb-2">
              <h4 className="text-sm font-medium text-gray-600">Filter by values:</h4>
            </div>

            {currentFilterColumn && filteredRequests.length > 0 && 
              getUniqueValues(filteredRequests, currentFilterColumn).map((value) => (
                <div key={value} className="my-1">
                  <FormControlLabel
                    control={
                      <Checkbox 
                        checked={filters[currentFilterColumn]?.includes(value) || false}
        ...
        <div className="md:hidden">
          ...
          <h2 className="text-xl font-bold text-blue-700 my-3">Corridor Requests</h2>
          {corridorRequests.length > 0 ? (
            corridorRequests.map((request) => (
              <div key={request.requestId} className="bg-white p-4 rounded-lg shadow-md mb-4 border border-gray-300">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="border border-gray-300 p-2"><strong>Corridor Type:</strong></div>
                  <div className="border border-gray-300 p-2">{request.corridorType}</div>
          ...
          <h2 className="text-xl font-bold text-green-700 my-3">Non-Corridor Requests</h2>
          {nonCorridorRequests.length > 0 ? (
            nonCorridorRequests.map((request) => (
              <div key={request.requestId} className="bg-white p-4 rounded-lg shadow-md mb-4 border border-gray-300">
                <div className="grid grid-cols-2 gap-2">
            ...
            <div className="text-center mb-5">No non-corridor requests found</div>
          )}
          
          {/* Emergency Requests Mobile View */}
          {emergencyRequests.length > 0 && (
            <>
              <h2 className="text-xl font-bold text-red-600 my-3">Emergency Block Requests</h2>
              {emergencyRequests.map((request) => (
                <div key={request.requestId} className="bg-white p-4 rounded-lg shadow-md mb-4 border border-gray-300">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="border border-gray-300 p-2"><strong>Request ID:</strong></div>
