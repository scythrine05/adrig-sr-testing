      ...
      if (info) {
        ...
        return (
          <div className="bg-white p-3 border rounded-md shadow-lg">
            ...
            <p><strong>Time:</strong> {formatTimeDisplay(info.startTime)} - {formatTimeDisplay(info.endTime)}</p>
            {info.department && <p><strong>Department:</strong> {info.department}</p>}
            <p className="text-xs mt-2 text-gray-500">Current station: {payload[0].payload.name}</p>
          </div>
        );
      }
    }
...
export default function GanttGraph({ data, scale = 1 }) {
  ...
  function findOverlappingBlockages(blockage, blockages) {
    return blockages.filter(b => 
      ...
      !( 
        ...
        (b.endTime <= blockage.startTime)    // b ends before blockage starts
      )
    );
  }
  ...
  data.forEach((element, ind) => {
    ...
    if (element.requests && element.requests.length > 0) {
      element.requests.forEach((request, requestIndex) => {
        ...
        const overlappingBlockages = findOverlappingBlockages(newBlockage, blockages);
  ...
  return <StationBlockageChart stations={stations} blockages={processedBlockages} scale={scale} />;
}
